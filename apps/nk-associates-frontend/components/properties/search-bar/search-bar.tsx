...
const SearchBar: FC<SearchBarProps> = ({ onFilter, isListView }) => {
  ...
  if (loading) {
    return <SearchBarSkeleton actListView={isListView} />;
  }

  return (
    <section
      className={`container relative flex flex-col ${isListView && "my-4"}`}
    >
      <div
      ...
      <div
        className={`hidden md:flex flex-wrap justify-center gap-0.5 overflow-hidden rounded-xl w-full shadow-3xl ${
          isListView && "my-6"
        }`}
      >
      ...
      <div
        className={`flex ml-auto ${
          !isListView && "absolute top-28 -right-16 z-10"
        }`}
      >
        {filtersState.filterIsSelected && (
          <button
